...
class AccountTests extends FunSuite {
  ...
  test(
    "Test 07: Correct balance amount after several withdrawals and deposits"
  ) {
...
class AccountTransferTests extends FunSuite {
  ...
  test("Test 08: Valid transfer between accounts") {
    ...
    assert(
      bank.completedTransactions.iterator.toList.last.getStatus == TransactionStatus.SUCCESS
    )
  ...
  test("Test 09: Transfer of negative amount between accounts should fail") {
    ...
    assert(
      bank.completedTransactions.iterator.toList.last.getStatus == TransactionStatus.FAILED
    )
